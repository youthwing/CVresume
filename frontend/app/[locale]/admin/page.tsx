              ...
              {orders.map((item) => (
                <div key={item.id} className="rounded-2xl border border-slate-200 p-4 text-sm dark:border-slate-800">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div>{formatPrice(item.amountCent, locale)}</div>
                  </div>
                  <div className="mt-1 text-slate-500 dark:text-slate-400">{item.userEmail} · {item.productType} · {item.credits} {sectionText(locale, "积分", "credits")}</div>
                </div>
              ))}
            </CardContent>
          </Card>
